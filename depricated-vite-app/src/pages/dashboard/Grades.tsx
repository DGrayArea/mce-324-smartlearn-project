...
const Grades = () => {
  ...
  const calculateGPA = () => {
    ...
    const totalPoints = gradedAssignments.reduce((sum, a) => sum + (a.grade! / a.maxGrade) * 4, 0);
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">
            {user?.role === 'lecturer' ? 'Grade Management' : 'My Grades & Results'}
          </h2>
                ...
                <CardHeader className="p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <CardDescription>
                        {isGrade ? `${item.course} • Student: ${item.student}` : item.course}
                      </CardDescription>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <div className="flex items-center">
                        ...
                        <span>
                          Submitted: {format(new Date(isGrade ? item.submissionDate : item.submissionDate!), 'MMM dd, yyyy')}
                        </span>
                      </div>
                      {isGrade && item.gradedDate && (
                        <div>
                          Graded: {format(new Date(item.gradedDate), 'MMM dd, yyyy')}
                        </div>
                      )}
                    </div>
