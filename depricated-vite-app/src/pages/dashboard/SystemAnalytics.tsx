...
const usageStatistics: UsageStatistic[] = [
  ...
  { category: 'Assignments Submitted', current: 8942, previous: 7834, unit: 'submissions' },
  { category: 'Video Meetings', current: 234, previous: 198, unit: 'meetings' },
  { category: 'Messages Sent', current: 15678, previous: 13245, unit: 'messages' },
  { category: 'File Downloads', current: 5432, previous: 4876, unit: 'downloads' }
];
...
const SystemAnalytics = () => {
  ...
  const getMetricIcon = (name: string) => {
    if (name.includes('CPU') || name.includes('Server')) return <Server className="h-4 w-4" />;
    if (name.includes('Memory') || name.includes('Storage')) return <Database className="h-4 w-4" />;
    if (name.includes('Network') || name.includes('Latency')) return <Wifi className="h-4 w-4" />;
    if (name.includes('Session') || name.includes('User')) return <Users className="h-4 w-4" />;
                ...
                <CardContent>
                  <div className="flex items-end justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span className={`text-xs ${metric.trend === 'up' ? 'text-green-600' : metric.trend === 'down' ? 'text-red-600' : 'text-blue-600'}`}>
            ...
            {usageStatistics.map((stat, index) => (
              <Card key={index}>
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className={`text-lg font-semibold ${
                        parseInt(calculateChange(stat.current, stat.previous)) > 0 ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {parseInt(calculateChange(stat.current, stat.previous)) > 0 ? '+' : ''}{calculateChange(stat.current, stat.previous)}%
                      </div>
                  ...
                  <div className="mt-4">
                    <Progress 
                      value={Math.min((stat.current / Math.max(stat.current, stat.previous)) * 100, 100)} 
                      className="h-2" 
                    />
                  </div>
                </CardContent>
            ...
            <CardContent>
              <div className="h-96 flex items-center justify-center text-muted-foreground">
                <div className="text-center">
                  ...
                  <p className="text-sm">Integration with charting library (e.g., Recharts) recommended</p>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Storage usage is at 82%. Consider cleaning up old files or adding more storage.
                    </p>
              ...
              <CardContent className="p-6">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      CPU usage has increased to 65%. Monitor for potential performance issues.
                    </p>
                    <p className="text-xs text-muted-foreground mt-2">
