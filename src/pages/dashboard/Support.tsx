  ...
  const [chatMessages, setChatMessages] = useState([
    {
      id: '1',
      sender: 'AI Assistant',
      content: "Hello! I'm here to help you with any questions about the platform. What can I assist you with today?",
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="help-center" className="space-y-6">
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-1 h-auto">
          <TabsTrigger value="help-center" className="flex-col sm:flex-row text-xs sm:text-sm p-2 sm:p-3">
            <HelpCircle className="h-3 w-3 sm:h-4 sm:w-4 sm:mr-2" />
            <span className="hidden sm:inline">Help Center</span>
            <span className="sm:hidden mt-1">Help</span>
          </TabsTrigger>
          <TabsTrigger value="faq" className="flex-col sm:flex-row text-xs sm:text-sm p-2 sm:p-3">
            <BookOpen className="h-3 w-3 sm:h-4 sm:w-4 sm:mr-2" />
            <span className="hidden sm:inline">FAQ</span>
            <span className="sm:hidden mt-1">FAQ</span>
          </TabsTrigger>
          <TabsTrigger value="chatbot" className="flex-col sm:flex-row text-xs sm:text-sm p-2 sm:p-3">
            <Bot className="h-3 w-3 sm:h-4 sm:w-4 sm:mr-2" />
            <span className="hidden sm:inline">AI Assistant</span>
            <span className="sm:hidden mt-1">AI</span>
          </TabsTrigger>
          <TabsTrigger value="tickets" className="flex-col sm:flex-row text-xs sm:text-sm p-2 sm:p-3">
          ...
          <TabsTrigger value="calendar" className="flex-col sm:flex-row text-xs sm:text-sm p-2 sm:p-3">
            <Calendar className="h-3 w-3 sm:h-4 sm:w-4 sm:mr-2" />
            <span className="hidden sm:inline">Schedule</span>
            <span className="sm:hidden mt-1">Schedule</span>
          </TabsTrigger>
          <TabsTrigger value="metrics" className="flex-col sm:flex-row text-xs sm:text-sm p-2 sm:p-3">
                  ...
                  {chatMessages.map((msg) => (
                    <div key={msg.id} className="flex items-start space-x-2">
                      <Avatar className="h-8 w-8">
                        <AvatarFallback>{msg.isBot ? 'AI' : (user ? `${user.firstName[0]}${user.lastName[0]}` : 'U')}</AvatarFallback>
                      </Avatar>
                      <div className={`rounded-lg p-3 max-w-xs ${msg.isBot ? 'bg-secondary' : 'bg-primary text-primary-foreground'}`}>
