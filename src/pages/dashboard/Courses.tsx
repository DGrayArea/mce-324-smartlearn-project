      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">
            {user?.role === 'lecturer' ? 'My Teaching Courses' : user?.role === 'admin' ? 'All Courses' : 'My Enrolled Courses'}
          </h2>
        ...
        <Button>
          ...
          {user?.role === 'lecturer' ? 'Create Course' : user?.role === 'admin' ? 'Add Course' : 'Enroll Course'}
        </Button>
      </div>
          ...
          <Card key={course.id} className="overflow-hidden">
            <CardHeader className="border-b bg-muted/40 p-4">
              <div className="flex justify-between items-start">
                ...
                <Badge variant={course.status === 'active' ? 'default' : course.status === 'completed' ? 'outline' : 'secondary'}>
                  {course.status}
                </Badge>
              </div>
            ...
            <CardContent className="p-4 space-y-3">
              ...
              {(user?.role === 'lecturer' || user?.role === 'admin') && course.students && (
                <div className="flex items-center text-xs text-muted-foreground">
